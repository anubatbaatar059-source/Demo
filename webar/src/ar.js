...
export async function initAR() {
  ...
  if (ZT.browserIncompatible()) { ZT.browserIncompatibleUI(); throw new Error("browser incompatible"); }
...
export function setSources(videoEl, webm="", mp4="", forceMP4=false){
  ...
  if (forceMP4 && mp4){
    ...
    const s=document.createElement("source"); s.src=webm; s.type='video/webm; codecs="vp9,opus"'; videoEl.appendChild(s);
  } else if (mp4){
    const s=document.createElement("source"); s.src=mp4; s.type="video/mp4"; videoEl.appendChild(s);
  }
...
export function fitPlaneToVideo(el){
  ...
  const baseH=0.9, W=baseH*(w/h);
  plane.geometry?.dispose?.(); plane.geometry=new THREE.PlaneGeometry(W, baseH);
  applyScale();
}
export function applyScale(){ if(!plane) return; plane.scale.set(scaleFactor, scaleFactor, 1); plane.position.set(0,0,0); }
export function faceCameraNoRotate(){ if(!plane||!camera) return; plane.quaternion.copy(camera.quaternion); plane.rotation.z = VIDEO_ROT_Z; }
...
export function makeSbsAlphaMaterial(tex){
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader:`precision highp float; uniform sampler2D map; varying vec2 vUv;
      void main(){ vec3 rgb = texture2D(map, vec2(vUv.x*0.5, vUv.y)).rgb;
...
export function localPointOnPlane(u,v){
  const w = plane.geometry.parameters.width, h = plane.geometry.parameters.height;
...
function hookGestures(){
  ...
  addEventListener("touchmove", e=>{
    if (e.touches.length===2 && plane){
      ...
      if (prev>0){ const ratio=k/prev; scaleFactor=Math.min(MAX_S,Math.max(MIN_S,scaleFactor*ratio)); applyScale(); }
      plane.dataset_prevDist=k;
    }
  }, {passive:true});
  addEventListener("touchend", ()=>{ if(plane) plane.dataset_prevDist=""; }, {passive:true});
  addEventListener("wheel", e=>{ scaleFactor=Math.min(MAX_S,Math.max(MIN_S,scaleFactor*(e.deltaY>0?0.95:1.05))); applyScale(); }, {passive:true});
}
